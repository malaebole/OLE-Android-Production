...
//    }
    ...
    public void pickPhotoClicked() {
        ...
        Permissions.check(getContext(), permissions, null /*rationale*/, null /*options*/, new PermissionHandler() {
            ...
            public void onGranted() {
                ...
                ImagePicker.with(OlePadelCertificateActivity.this)
                        .crop() // Optionally add crop support
                        .compress(1024) // Image compression size in KB
                        .maxResultSize(1080, 1080) // Maximum image dimensions
                        .start();
            }
        });
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == ImagePicker.REQUEST_CODE && resultCode == RESULT_OK) {
            // Handle the image picked by ImagePicker
            Uri imageUri = data.getData();
            if (imageUri != null) {
                try {
                    // Obtain the file path from the URI
                    String path = getFilePathFromUri(getContext(), imageUri);
                    if (path != null) {
                        file = new File(path);
                        binding.tvFileName.setText(file.getName());
                        binding.nameVu.setVisibility(View.VISIBLE);
                    } else {
                        Functions.showToast(getContext(), "Unable to retrieve file path", FancyToast.ERROR);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    Functions.showToast(getContext(), "Error processing file", FancyToast.ERROR);
                }
            }
            ...
            if (uri != null) {
                try {
                    ...
                    if (path == null) {
                        path = getFilePathFromUri(getContext(), uri);
                    }
                    file = new File(path);
                    if (file != null) {
